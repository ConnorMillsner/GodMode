...
export default function TransformationPage() {
  ...
  useEffect(() => {
    ...
    if (score && suggestions && imageUrl) {
      try {
        ...
        const parsedObservations = JSON.parse(decodeURIComponent(observations));
        const parsedImageUrl = decodeURIComponent(imageUrl);
        
        setTransformationData({
          originalScore: parseInt(score),
          transformedScore: 0,
        ...
        generateTransformation(parsedSuggestions, parsedObservations, parseInt(score), parsedImageUrl);
  ...
  const generateTransformation = async (suggestions: string[], observations: string[], score: number, originalImageUrl: string) => {
    try {
      ...
      if (data.success) {
        setTransformationData(prev => prev ? {
          ...prev,
      ...
      setTransformationData(prev => prev ? {
        ...
        fallbackMessage: 'Transformation generation is temporarily unavailable. Your potential improvement would be significant.',
  ...
  const handleBackToResults = () => {
    ...
    const resultsParams = new URLSearchParams({
      ...
      suggestions: encodeURIComponent(JSON.stringify(transformationData.suggestions)),
      observations: encodeURIComponent(JSON.stringify(transformationData.observations)),
            ...
            <div className={styles.imageWrapper}>
              ...
              <div className={styles.imageLabel}>
                ...
                <div className={styles.scoreDisplay}>
                  <span className={styles.score}>{transformationData.originalScore}</span>
          ...
          <div className={styles.imageContainer}>
            <div className={styles.imageWrapper}>
              ...
              <div className={styles.imageLabel}>
                <h3>After</h3>
                <div className={styles.scoreDisplay}>
                  <span className={styles.score}>{transformationData.transformedScore}</span>
                  <span className={styles.scoreMax}>/100</span>
                </div>
              </div>
            </div>
        ...
        <div className={styles.improvementSummary}>
          ...
          <div className={styles.scoreImprovement}>
            <span className={styles.scoreIncrease}>
              +{transformationData.transformedScore - transformationData.originalScore} points
            </span>
            <span className={styles.improvementText}>
              ({transformationData.originalScore} → {transformationData.transformedScore})
            </span>
          </div>
        </div>
      </div>
